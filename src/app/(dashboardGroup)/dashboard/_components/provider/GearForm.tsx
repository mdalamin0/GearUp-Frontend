...
import { updateGear } from "../../_actions/provider/updateGear";
...
const GearForm = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      rentalPrice: Number(gear?.rentalPrice) || 1,
      stock: gear?.stock || 1,
      image: gear?.image || "",

      length: (gear?.specifications?.length as string) || "",
      maxLoad: (gear?.specifications?.maxLoad as string) || "",
      material: (gear?.specifications?.material as string) || "",
      weight: (gear?.specifications?.weight as string) || "",
    },
  });
  ...
  const onSubmit = async (values: TGearForm) => {
    ...
    try {
      if (mode === "create") {
        ...
        const res = await updateGear(gear!.id, payload);
        if (res.success) {
          toast.success(res.message);
